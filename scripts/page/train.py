...
OUTPUT_DIR = "outputs/page/qwen3.5-4B"
...
class QwenDataCollator:
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            # Load the image lazily in the collator to avoid Dataset serialization issues
        ...
        # For full-page, we might need to handle higher resolution or different padding
        batch = self.processor(
            ...
            padding=True,
            min_pixels=512*512,
            max_pixels=1344*1344, # Increased to ~1.8 Megapixels to use the extra 8GB VRAM for better OCR clarity
        )

        # 3. Create labels (cloned from input_ids)
        labels = batch["input_ids"].clone()
        
        # 4. Define our markers for masking
...
def train():
    ...
    if torch.cuda.is_available():
        ...
        print("No CUDA GPU detected! This script requires a GPU server.")
        return

    # 1. Load Model & Processor
    # For full-page, we use the same 4B model but we will fine-tune the vision layers
    ...
    # 2. Add LoRA Adapters
    # CRITICAL: finetune_vision_layers = True for full-page OCR
    model = FastVisionModel.get_peft_model(
        model,
        finetune_vision_layers     = True,  # Enabled for better spatial awareness
    ...
    # 3. Load Dataset
    print("Loading synthetic full-page dataset...")
    # Point to the metadata file generated by generate_synthetic.py
    metadata_path = "data/synthetic_train/metadata.jsonl"
    
    if os.path.exists(metadata_path):
        dataset = load_dataset("json", data_files=metadata_path, split="train")
        # Store absolute path to load efficiently in the collator
        def add_path(example):
            example["image_path"] = os.path.join("data/synthetic_train", example["image"])
            return example
        dataset = dataset.map(add_path)
    else:
        print(f"Metadata not found at {metadata_path}. Falling back to Teklia/IAM-line reference.")
        try:
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=4,  # Pushed to 4 to utilize remaining ~15GB VRAM
        gradient_accumulation_steps=16, # Maintain effective batch size of 64
        warmup_steps=50,
        max_steps=500,
        learning_rate=1e-4, # Slightly lower LR for vision layer stability
    ...
    trainer = SFTTrainer(
        ...
        max_seq_length=4096, # Increased to 4096 to accommodate higher-res 1344x1344 images + text
    )
    ...
    # Start Training
    print("Starting full-page OCR training...")
    trainer.train()
